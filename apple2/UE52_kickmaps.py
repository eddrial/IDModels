...
def loopdeloop(gap,shift,shiftmode):
    ...
    rd.UtiDelAll()
    UE52_params = parameters.model_parameters(Mova = 0,
                                        periods = 10, 
                                        periodlength =52,
    ...
    UE52 = id1.plainAPPLE(UE52_params)
    
    UE52.cont.wradSolve()
    
    fileheader = 'Undulator {}, Gap {}mm, Shift {}mm, Mode {}'.format(UE52_params.periodlength,
                                                                      gap,
                                                                      shift,
                                                                      shiftmode)
    
    a = rd.FldFocKickPer(UE52.cont.radobj, [0,0,0],[0,1,0],52,74,[1,0,0],30,61,6,25,'fileheader',[5,8,0,0])#,'T2m2',1.72,'tab')
    with open('D:/Results/UE52/kick_maps/kick_{}l_{}g_{}s_{}m.txt'.format(UE52_params.periodlength,
                                                                      gap,
                                                                      shift,
                                                                      shiftmode), 'w') as f:
        f.write(a[5])
    
    figtitle = 'UE{} ¦¦ Mode: {} ¦¦ Gap: {}mm 11 Shift {}mm'.format(UE52_params.periodlength,
                                                                  shiftmode,
                                                                  gap,
                                                                  shift
                                                                  )
    ...
    b.savefig('D:/Results/UE52/kick_maps/kickmap_{}l_{}g_{}s_{}m.png'.format(UE52_params.periodlength,
                                                                      gap,
                                                                      shift,
...
def makevid(gap, shifts, shiftmode):
    image_folder = 'D:/Results/UE52/kick_maps/'
    video_name = 'UE52_Kicks_{}_mode_{}mm_gap.avi'.format(shiftmode, gap)
    
    
    image_ref = '{}kickmap_52l_15g_25s_circularm.png'.format(image_folder)
    ...
    for shift in shifts:
        print(shift)
        image = 'kickmap_{}l_{}g_{}s_{}m.png'.format(52,
                                                      gap,
                                                      shift,
                                                      shiftmode)
